...
elapsedMillis conversion_rate = 0;
  ...
  int accumulateMode = false;    //***IS THIS NECESSARY NOW THAT THE ADS8321 IS BEING USED?****
  int i;
...
// Step 3: Integration time -- sample for a period of time determined by the parameter
...
// Step 4: Read Data 2 (this is the actual read, since the spectrometer has now sampled data)
  int idx = 0;
  for (int i = 0; i < SPEC_CHANNELS; i++) {    
    // Four clocks per pixel
    // First block of 2 clocks -- measurement
...
// Step 6: Send start pulse to signal end of integration/light collection
  digitalWriteFast(SPEC_CLK, LOW);
  delayMicroseconds(delay_time);
  digitalWriteFast(SPEC_CLK, HIGH);
  digitalWriteFast(SPEC_ST, LOW);
